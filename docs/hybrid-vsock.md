...
Silo exposes virtio-vsock through a host Unix-socket surface. silo-vmmon always
...
socketpair inherited by the `silo-krun` worker (fd 7 of its fixed descriptor
...
silo-vmmon replies `OK <host-source-port>\n` after the guest accepts, then relays
...
effective UID as silo-vmmon. Malformed, oversized, or unauthorized requests are
...
The extension, not silo-vmmon, owns the listener lifecycle:
...
3. Accept relayed Unix streams from silo-vmmon.
4. Close and attempt to unlink the listener during extension shutdown, treating
   an already-removed path as success.

silo-vmmon watches the runtime directory and registers valid listeners dynamically.
...
Only socket entries in silo-vmmon's owner-only runtime directory are discovered.
...
silo-vmmon stops admitting work during shutdown, resets backend streams, drains
relays, and removes the mux and private backend sockets it owns. After
silo-vmmon exits, libvm removes the complete machine runtime tree, including names for
