...
`uds` while disabled is rejected. This is an intentional schema break without
a VM spec version bump: old endpoint and plugin fields are rejected with a
migration error rather than ignored.
...
4. Close and unlink the listener during extension shutdown.
...
relays, and removes the mux and private backend sockets it owns. It does not
unlink extension-owned `<mux>_<port>` listeners. Extensions must close those
listeners and retry or fail outstanding work when the VM exits.
