...
`silo-vmmon` is the VM monitor: one process per running VM generation. It is
not a VMM. The VMM is libkrun or Apple's Virtualization.framework; the monitor
starts it, serves the machine's control API and guest services, and records how
the generation ended. Its interface with libvm is
[ADR 0018](../adr/0018-silo-vmmon-contract.md); this document describes how it
is built.
...
```text
libvm / CLI / SDK
  -> silo-vmmon                    daemonized, one per generation
       Tokio, control API, guest services, forwards, finalization
       |
       +-> silo-krun               krun backend: same executable, argv[0] "silo-krun"
...
`silo-krun` it runs the worker (`virt/vmmon/src/krun/worker`) and never touches
supervisor code: no argument parsing, daemonization, tracing or Tokio. On Linux
the worker also sets its thread name, so `/proc/<pid>/comm` reads `silo-krun`.
`ps` shows the worker as exactly `silo-krun` on both platforms.

libkrun is linked into silo-vmmon, but VM construction and execution happen
...
in process on its own dispatch queue. On macOS silo-vmmon is signed with both
`com.apple.security.hypervisor` and `com.apple.security.virtualization`
(`virt/vmmon/silo-vmmon.entitlements`).

## Worker Handoff

The monitor spawns its own executable with argv[0] `silo-krun` and no
...
directory, the executable (to start `silo-krun`), and the Mach services and
...
| Path | Contents |
| --- | --- |
| `virt/vmmon/src/main.rs` | argv[0] dispatch, argument parsing, daemonization, exit runner fork, tracing, runtime. |
| `virt/vmmon/src/startup.rs`, `services.rs`, `shutdown.rs` | Start request, backend construction, guest services, shutdown triggers. |
| `virt/vmmon/src/finalize.rs` | The single terminal path. |
| `virt/vmmon/src/exit_command.rs`, `exit_status.rs` | Exit runner and `vm.exit.json`. |
| `virt/vmmon/src/virt/` | Backend-neutral facade, `VmExit`, krun owner and mux, VZ backend. |
| `virt/vmmon/src/krun/` | `KrunConfig`, engine, host checks, Rosetta data. |
| `virt/vmmon/src/krun/worker/` | Worker entry, descriptor adoption (`fds.rs`), wire format (`wire.rs`), HVF admission. |

See [process and native test instructions](../../virt/vmmon/tests/README.md),
...
| Signed macOS HVF native gate: devices, serial RPC, block and virtio-fs, stop, worker SIGKILL, reboot, poweroff, new generation, worker argv `silo-krun` | Passed |
| Linux KVM native gate, including x86-64 poweroff with a kernel built from this tree | Not yet run |
| Linux `/proc/<pid>/comm` is `silo-krun` | Not yet run |
