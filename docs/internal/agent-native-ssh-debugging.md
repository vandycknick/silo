...
Current builds obtain `AgentConfig` from `silo-vmm` metadata. Confirm the VM booted
with the rebuilt initramfs and inspect `silo-vmm` and agent logs for metadata
