# 8. silo-vmm Host and Guest Agent gRPC APIs
...
That interface spans two processes. `silo-vmm` owns the VMM, lifecycle policy,
...
`silo-vmm` API, while `silo-vmm` uses the guest agent API.
...
- Each `silo-vmm` serves a host gRPC API on its machine-scoped Unix socket.
- The guest agent serves a guest gRPC API on machine-scoped vsock port 1027.
- `silo-vmm` initiates all guest API connections. The guest never calls back into
  the host API.
...
validated by `silo-vmm`, forwarded to the guest service, and independently
...
`silo-vmm` uses `WatchStatus` and `WatchMetrics` during normal supervision. The
...
One `silo-vmm` invocation owns exactly one VMM instance and one VM boot. It creates
one random monitor instance ID and one in-memory state store. A terminal VM
state ends that process; restarting a machine creates a new `silo-vmm`, monitor
...
After the VM reaches the VMM's running state, `silo-vmm` starts independent status
and metrics supervisors.

### Status Stream

`silo-vmm` opens `WatchStatus` with a five-second heartbeat interval. The guest
...
a different instance ID, `silo-vmm` clears status and metrics from the previous
...
For every accepted status or metric message, `silo-vmm` records:
...
are disabled, that condition is sufficient. If they are enabled, `silo-vmm` also
...
proxied by `silo-vmm`. This gives callers typed filesystem operations before guest
...
`silo-vmm` validates host requests before connecting to the guest. The guest
validates the request again before touching the filesystem. For reads, `silo-vmm`
...
`silo-vmm` does not retry a filesystem mutation after delivery becomes ambiguous.
...
gRPC framing and flow control only; `silo-vmm` does not parse, authenticate, log,
or modify the relayed SSH or terminal protocol.

Before returning a successful RPC response, `silo-vmm` acquires stream capacity
...
The host SSH client, not `silo-vmm`, owns SSH server-key acceptance and continuity.
...
When proxying filesystem operations, `silo-vmm` ensures a detail-free guest status
...
socket is created with mode `0600`. `silo-vmm` also requires each accepted peer UID
...
lifecycle operations. At startup, `silo-vmm` tightens the machine directory,
removes the expected stale socket entry, binds the socket, and sets its
permissions. `silo-vmm` does not unlink the path during late process exit, avoiding
removal of a successor's listener. Managed path reconciliation remains a
`libvm` responsibility.

### Guest Agent

The guest accepts RPC connections only from the host vsock CID. `silo-vmm` opens
...
`silo-vmm` therefore validates guest messages after protobuf decoding, bounds all
...
On requested shutdown, `silo-vmm`:
...
- Long-lived watches require reconnect, silence detection, cancellation, and
  identity-reset logic in `silo-vmm`.
...
SFTP or SSH exec would require `silo-vmm` to terminate SSH, own guest credentials,
...
- Status, metrics, and diagnostics are not retained after `silo-vmm` exits.
...
- SSH parsing, termination, authentication, or server-key policy in `silo-vmm`;
- sanitization of terminal or file content for client display;
- in-process VM restart or multiple VM boots in one `silo-vmm` process;
...
- `virt/vmm/src/services.rs` for host service admission, access streams,
  health, and reflection
- `virt/vmm/src/guest.rs` for guest watches, discovery, and reconnects
- `virt/vmm/src/state.rs` for identity, validation, freshness, and
  readiness
- `virt/vmm/src/filesystem.rs` for the host filesystem proxy
