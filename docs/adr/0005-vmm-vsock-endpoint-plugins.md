# 5. silo-vmm Endpoint Plugins for Vsock Streams
...
`silo-vmm` already owns per-VM runtime supervision. It reads `config.json`, starts the VM, exposes the monitor control surface, and tracks VM and guest readiness.

Today, host to guest stream integrations are implemented as built-ins. `silo-vmm` already connects to guest vsock ports for things like the guest agent and shell access. That works for a small fixed set of features, but it does not scale well for arbitrary host to guest services. Every new service would require more built-in `silo-vmm` logic, more coupling to host virtualization details, and more monitor-specific code for behavior that does not belong in the core VM supervisor.

Silo needs a generic way for `silo-vmm` to launch long-running helpers that can consume host to guest byte streams without `silo-vmm` relaying data in userspace.
...
Silo will add declarative `vsock_endpoints` to `VmSpec` and `silo-vmm` will supervise vsock endpoint plugins for them.
...
Plugins are external processes launched by `silo-vmm`.
...
- `silo-vmm` creates a Unix `socketpair` control socket,
- `silo-vmm` passes one end of that control socket to the plugin as `fd 3`,
- the plugin requests guest streams on demand over the control socket,
- `silo-vmm` opens a connected vsock stream to the configured guest port for each request,
- `silo-vmm` passes each connected stream back to the plugin over the control socket using `SCM_RIGHTS`.

For `listen` mode:

- `silo-vmm` creates a Unix `socketpair`,
- `silo-vmm` passes one end of that control socket to the plugin as `fd 3`,
- `silo-vmm` accepts guest-initiated vsock connections,
- `silo-vmm` passes each accepted connected stream to the plugin over the control socket using `SCM_RIGHTS`.

This keeps `silo-vmm` out of the data path while allowing one long-running plugin process to handle multiple guest connections in both modes.
...
sequenceDiagram
  ...
  participant monitor as silo-vmm
  participant virt as virt
  participant plugin as plugin

  libvm->>monitor: spawn silo-vmm (startup-fd pipe)
...
Endpoint failures are handled by `silo-vmm` supervision, restart policy, and logs, not by redefining overall instance readiness.
...
- `silo-vmm` stays generic and does not need service-specific built-ins for every new host to guest integration.
- Plugins can be written against one small API with no driver-specific logic.
- `silo-vmm` avoids becoming a per-byte relay in the hot data path.
...
- `silo-vmm` gains process supervision logic for plugins.
- `silo-vmm` must own fd hygiene, child process setup, restart policy, and stdout protocol parsing.
...
- `mode` defines whether `silo-vmm` connects or listens.
- `plugin` describes the executable launched by `silo-vmm`.
...
`silo-vmm` writes exactly one JSON object to plugin stdin as a single UTF-8 line.
...
- `silo-vmm` uses that control socket to exchange fixed-size control messages and optional `SCM_RIGHTS` fd attachments.
...
- `silo-vmm` responds by opening a new vsock connection to the configured endpoint port,
- `silo-vmm` returns that connected stream fd via `SCM_RIGHTS`.

#### `mode: listen`

- plugins call `Plugin::accept().await?` to wait for the next guest-initiated connection,
- `silo-vmm` accepts guest-initiated connections on the configured endpoint port,
- `silo-vmm` returns each accepted stream fd via `SCM_RIGHTS`.
...
`silo-vmm` owns endpoint supervision internally and treats plugin readiness/failure events as process-supervision inputs. Scripts that need endpoint definitions should read the VM spec, not monitor inspection output.
