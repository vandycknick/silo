...
one-shot `silo-vmmon` exit watcher. The watcher receives the exact run generation,
...
`silo-vmmon` acknowledges the start only after the guest program launches, then
