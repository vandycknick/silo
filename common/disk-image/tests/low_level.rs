...
use disk_image::ext4::error::FormatError;
use disk_image::ext4::types::{GroupDescriptor, SuperBlock};
use disk_image::ext4::{Formatter, Reader, extent};
...
// ===========================================================================
// Test: inode dtime after removal
// ===========================================================================

#[test]
fn test_inode_dtime_after_removal() {
    let (mut fmt, tmp) = new_formatter();
    ...
    // After removal the formatter zeros the inode but sets dtime.
    assert_eq!(victim.links_count, 0, "removed inode links_count must be 0");
    assert_ne!(
        victim.dtime, 0,
        "removed inode dtime must be non-zero (deletion timestamp)"
    );
}
...
#[test]
fn test_removing_hardlink_does_not_decrement_parent_directory_link_count() {
    let (mut fmt, tmp) = new_formatter();
    ...
    assert_eq!(
        root.links_count, 3,
        "removing a hardlink entry must not decrement the parent directory link count"
    );
