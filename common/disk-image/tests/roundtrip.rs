...
#[test]
fn test_unicode_paths_and_symlinks() {
    let (mut fmt, tmp) = new_formatter();
    ...
    assert_eq!(
        reader.read_file("/data/\u{94fe}\u{63a5}", 0, None).unwrap(),
        b"hello"
    );
    assert!(is_link(
        reader
...
fn test_remove_operations() {
    ...
    fmt.create(
        ...
        None,
        None,
        Some(&mut b"second".as_slice()),
        None,
        None,
        None,
    )
    .unwrap();
    fmt.remove_children("/directory").unwrap();
    fmt.create(
        "/remove-me",
        make_mode(file_mode::S_IFREG, 0o644),
        None,
        None,
        Some(&mut b"gone".as_slice()),
        None,
        None,
        None,
    )
    .unwrap();
    fmt.remove("/remove-me").unwrap();
    fmt.close().unwrap();

    let mut reader = Reader::new(tmp.path()).unwrap();
    assert!(reader.exists("/directory"));
    assert!(reader.list_dir("/directory").unwrap().is_empty());
    assert!(!reader.exists("/remove-me"));
}
...
fn test_read_nonexistent_path() {
    ...
    assert!(
        matches!(err, disk_image::ext4::error::ReadError::PathNotFound(_)),
...
fn test_read_file_on_directory() {
    ...
    assert!(
        matches!(err, disk_image::ext4::error::ReadError::IsDirectory(_)),
...
fn test_list_dir_on_file() {
    ...
    assert!(
        matches!(err, disk_image::ext4::error::ReadError::NotADirectory(_)),
...
fn test_invalid_image() {
    ...
    match Reader::new(tmp_zeros.path()) {
        Err(disk_image::ext4::error::ReadError::InvalidSuperBlock) => {} // expected
...
fn test_symlink_loop_detection() {
    ...
    assert!(
        matches!(err, disk_image::ext4::error::ReadError::SymlinkLoop(_)),
