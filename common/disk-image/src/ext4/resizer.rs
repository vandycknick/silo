...
use crate::ext4::constants::{self, bg_flags, compat, file_mode, incompat, ro_compat};
use crate::ext4::error::{ResizeError, ResizeResult};
use crate::ext4::extent;
use crate::ext4::journal;
use crate::ext4::layout;
use crate::ext4::types::{GroupDescriptor, Inode, SUPERBLOCK_SIZE, SuperBlock};
...
mod tests {
    ...
    use crate::ext4::resizer::{BLOCK_SIZE, write_superblock};
    use crate::ext4::{Formatter, Reader, grow_image};
    ...
    fn grows_formatted_image_and_preserves_contents() {
        ...
        formatter
            .create(
                "/hello",
                crate::ext4::constants::make_mode(
                    crate::ext4::constants::file_mode::S_IFREG,
                    0o644,
                ),
                None,
                None,
    ...
    fn refuses_dirty_journal_without_modifying_image() {
        ...
        assert!(matches!(
            error,
            crate::ext4::error::ResizeError::RequiresRecovery
        ));
    ...
    fn rejects_target_beyond_32_bit_blocks_without_modifying_image() {
        ...
        assert!(matches!(
            error,
            crate::ext4::error::ResizeError::TooLarge { .. }
        ));
        assert_eq!(std::fs::metadata(&path).unwrap().len(), before_size);
        let mut after_superblock = [0u8; 1024];
        let mut file = std::fs::File::open(&path).expect("open image");
    ...
    fn rejects_reserved_gdt_span_larger_than_pointer_block() {
        ...
        assert!(matches!(
            error,
            crate::ext4::error::ResizeError::Corrupt(
                "resize inode GDT span exceeds double-indirect block"
            )
        ));
        assert_eq!(std::fs::metadata(&path).unwrap().len(), before_size);
