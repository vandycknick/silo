...
impl<S: AsRawFd + ReadVolatile + Write + WriteVolatile + IsHybridVsock> VsockConnection<S> {
        ...
        match self.rx_queue.dequeue() {
            ...
            Some(RxOps::Rw) => {
                ...
                match self.stream.read_volatile(&mut buf) {
                    ...
                    Err(vm_memory::VolatileMemoryError::IOError(error))
                    ...
                    Err(error) => {
                        log::debug!("Failed to read host vsock stream: {error:?}");
                        pkt.set_op(VSOCK_OP_RST);
                        self.rx_queue.enqueue(RxOps::Reset);
                    }
                }
    ...
    pub fn send_pkt<B: BitmapSlice>(&mut self, pkt: &VsockPacket<B>) -> Result<()> {
        ...
        match pkt.op() {
            ...
            VSOCK_OP_SHUTDOWN => {
                ...
                let shutdown = match (recv_off, send_off) {
                ...
                if let Some(shutdown) = shutdown {
                    let _ = self.stream.shutdown(shutdown);
                }
    ...
    fn send_bytes<B: BitmapSlice>(&mut self, buf: &VolatileSlice<B>) -> Result<()> {
        ...
        if written_count != buf.len() {
            ...
            let remaining = buf
                ...
                .map_err(|_| Error::PktBufMissing)?;
            return self.tx_buf.push(&remaining);
        }
    ...
    ) -> Result<&'a mut VsockPacket<'b, B>> {
        // Zero out the packet header
        pkt.set_header_from_raw(&[0u8; PKT_HEADER_SIZE])
            .map_err(|_| Error::PktBufMissing)?;
...
mod tests {
    ...
    impl HeadParams {
        ...
        fn construct_head(&self) -> Vec<u8> {
            ...
            if self.head_len == PKT_HEADER_SIZE {
                ...
                header[HDROFF_LEN..HDROFF_LEN + std::mem::size_of::<u32>()]
    ...
    impl IsHybridVsock for VsockDummySocket {
        ...
        fn shutdown(&self, _how: Shutdown) -> IoResult<()> {
            Ok(())
        }
    }
    ...
    fn test_vsock_conn_init_pkt() {
        ...
        conn_local.init_pkt(&mut pkt).expect("initialize packet");
    ...
    #[test]
    fn closed_host_before_guest_response_queues_reset() {
        let (backend, host) = UnixStream::pair().expect("create stream pair");
        drop(host);
        let mut connection = VsockConnection::new_local_init(
        ...
        packet.set_op(VSOCK_OP_RESPONSE);

        connection
            .send_pkt(&packet)
            .expect("consume guest response");

        assert!(connection.rx_queue.contains(RxOps::Reset.bitmask()));
        assert!(!connection.connect);
    }

    #[test]
    fn guest_shutdown_send_closes_host_read_half() {
        let (backend, mut host) = UnixStream::pair().expect("create stream pair");
        let mut connection = VsockConnection::new_local_init(
            StreamType::Unix(backend),
            VSOCK_HOST_CID,
            1 << 30,
        ...
        packet
            ...
            .set_flags(VSOCK_FLAGS_SHUTDOWN_SEND);

        connection
            .send_pkt(&packet)
            .expect("consume guest shutdown");

        let mut byte = [0_u8; 1];
        assert_eq!(host.read(&mut byte).expect("read host EOF"), 0);
    }
}
