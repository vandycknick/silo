...
impl LocalTxBuf {
    ...
    pub fn push<B: BitmapSlice>(&mut self, data_buf: &VolatileSlice<B>) -> Result<()> {
        ...
        if len < data_buf.len() {
            ...
            data_buf
                ...
                .map_err(|_| Error::LocalTxBufFull)?;
        }
    ...
    pub fn flush_to<S: Write>(&mut self, stream: &mut S) -> Result<usize> {
        ...
        Ok(written + self.flush_to(stream)?)
    }
...
mod tests {
    ...
    #[test]
    fn wrapped_flush_propagates_second_write_failure() {
        struct FailSecondWrite(bool);
        ...
        let mut buffer = LocalTxBuf::new(4);
        buffer.buf.copy_from_slice(&[1, 2, 3, 4]);
        buffer.head = Wrapping(2);
        buffer.tail = Wrapping(6);

        assert!(matches!(
            buffer.flush_to(&mut FailSecondWrite(false)),
            Err(Error::LocalTxBufFlush(error)) if error.kind() == io::ErrorKind::BrokenPipe
        ));
    }
}
