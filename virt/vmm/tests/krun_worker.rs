...
fn command() -> Command {
    let mut command = Command::new(env!("CARGO_BIN_EXE_silo-vmm"));
    command
        .process_group(0)
        .stdin(Stdio::null())
        .stdout(Stdio::piped())
...
impl SupervisorFixture {
    fn new() -> Self {
        ...
        let root =
            std::path::Path::new("/tmp").join(format!("silo-vmm-worker-{}", uuid::Uuid::new_v4()));
...
#[test]
fn executable_and_worker_have_distinct_names() {
    let help = command().arg("--help").output().expect("supervisor help");
    assert!(help.status.success());
    assert!(String::from_utf8_lossy(&help.stdout).contains("silo-vmm"));

    let (process, _config, keepalive, _events, _console) = worker();
    let pid = process.0.as_ref().expect("worker").id();
    let argv = Command::new("ps")
        .args(["-p", &pid.to_string(), "-o", "args="])
        .output()
        .expect("inspect worker command line");
    assert!(argv.status.success());
    assert_eq!(String::from_utf8_lossy(&argv.stdout).trim(), "krun");
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        loop {
            let name = std::fs::read_to_string(format!("/proc/{pid}/comm"))
                .expect("read worker process name");
            if name.trim() == "krun" {
                break;
            }
            assert!(Instant::now() < deadline, "worker name was {name:?}");
            std::thread::sleep(Duration::from_millis(10));
        }
        let executable =
            std::fs::read_link(format!("/proc/{pid}/exe")).expect("read worker executable");
        assert_eq!(executable.file_name().expect("executable name"), "silo-vmm");
    }

    drop(keepalive);
    assert_eq!(process.output().status.code(), Some(125));
}
...
#[tokio::test]
#[ignore = "requires native hypervisor and SILO_TEST_KERNEL/INITRAMFS/SHUTDOWN; macOS silo-vmm must be signed"]
...
async fn native_guest_cases(scenarios: &[NativeExit]) {
    ...
    for (generation, &scenario) in scenarios.iter().enumerate() {
        ...
        assert_eq!(
            ...
            WORKER_NAME,
            "worker argv is exactly [krun]"
        );
        #[cfg(target_os = "linux")]
        {
            assert_eq!(
                std::fs::read_link(format!("/proc/{worker}/exe")).expect("worker executable"),
                std::fs::canonicalize(env!("CARGO_BIN_EXE_silo-vmm")).expect("silo-vmm executable")
            );
            assert_eq!(
