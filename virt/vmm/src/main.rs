...
#[command(name = "silo-vmm", disable_help_subcommand = true)]
struct Args {
...
fn main() -> eyre::Result<()> {
    ...
    tracing_subscriber::fmt()
        ...
        .with_writer(writer)
        .try_init()
        .map_err(|err| eyre::eyre!("initialize silo-vmm tracing: {err}"))?;

    tracing::info!(
        event = "generation_start",
        machine_id = %args.id,
        run_id = %args.run_id,
        "silo-vmm generation started"
    );
...
fn write_serial_generation_boundary(
    ...
    writeln!(
        file,
        "--- silo-vmm generation start machine_id={machine_id} run_id={run_id} ---"
    )
...
mod tests {
    ...
    fn serial_generation_boundary_is_stable_and_identifiable() {
        let path = std::env::temp_dir().join(format!("silo-vmm-boundary-{}", uuid::Uuid::new_v4()));
        ...
        assert_eq!(
            contents,
            "--- silo-vmm generation start machine_id=machine-1 run_id=run-1 ---\n"
        );
