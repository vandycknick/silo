...
pub(crate) fn check_hvf() -> Result<krun::HvfHostInfo, HvfAdmissionError> {
    ...
    // private HvfVm. Call the public SDK API directly so this remains binary-only.
...
fn status_guidance(status: i32) -> &'static str {
    match status {
        HV_DENIED => {
            " Hint: rebuild the signed helper with `cargo run --locked -p xtask -- component krun` to grant com.apple.security.hypervisor, and verify sandbox policy permits Hypervisor.framework"
        }
...
mod tests {
    use crate::admission::{status_guidance, HvfStatus, HV_DENIED, HV_NO_RESOURCES};
    ...
    fn denied_status_names_the_required_entitlement() {
        ...
        assert!(
            status_guidance(HV_DENIED).contains("cargo run --locked -p xtask -- component krun")
        );
    }
