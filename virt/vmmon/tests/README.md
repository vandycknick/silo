# silo-vmmon process tests

Run the real executable boundary tests without a working hypervisor:

```sh
cargo test --locked -p silo-vmmon --test krun_worker -- --nocapture
cargo test --locked -p silo-vmmon --all-features --bin silo-vmmon krun::worker::
cargo test --locked -p silo-vmmon --all-features --bin silo-vmmon virt::backend::krun::
cargo test --locked -p silo-vmmon --all-features --bin silo-vmmon exit_command::
cargo test --locked -p silo-vmmon --all-features --bin silo-vmmon virt::serial::tests::
...
`CARGO_BIN_EXE_silo-vmmon` identifies the actual executable, never the Rust
unit-test harness. The worker is that same executable spawned with argv[0]
`silo-krun`, no arguments, and the fixed descriptor table described in
...
rustc --edition=2024 --target "$GUEST_TARGET" \
  ...
  virt/vmmon/tests/guest/shutdown.rs -o target/krun-guest-shutdown
...
SILO_TEST_SHUTDOWN="$PWD/target/krun-guest-shutdown" \
  cargo test --locked -p silo-vmmon --test krun_worker \
...
```sh
rustc --edition=2024 --test virt/vmmon/tests/guest/shutdown.rs -o target/krun-guest-shutdown-tests
target/krun-guest-shutdown-tests
```

The musl Rust target must already be available. On macOS, build the test executable
first with `cargo test --locked -p silo-vmmon --test krun_worker --no-run`, sign
`target/debug/silo-vmmon` with `virt/vmmon/silo-vmmon.entitlements`, and run
...
vsock/balloon device enumeration, a single `silo-krun` child whose argv is exactly
`silo-krun` (and whose executable is `silo-vmmon` on Linux), the `backend` block
...
See [silo-vmmon architecture](../../../docs/architecture/silo-vmmon.md) for
