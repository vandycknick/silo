...
fn command() -> Command {
    let mut command = Command::new(env!("CARGO_BIN_EXE_silo-vmmon"));
    command
        .process_group(0)
        .stdin(Stdio::null())
        .stdout(Stdio::piped())
...
impl SupervisorFixture {
    fn new() -> Self {
        ...
        let root = std::path::Path::new("/tmp")
            .join(format!("silo-vmmon-worker-{}", uuid::Uuid::new_v4()));
...
#[tokio::test]
#[ignore = "requires native hypervisor and SILO_TEST_KERNEL/INITRAMFS/SHUTDOWN; macOS silo-vmmon must be signed"]
    ...
    for (generation, &scenario) in scenarios.iter().enumerate() {
        ...
        #[cfg(target_os = "linux")]
        {
            assert_eq!(
                ...
                std::fs::canonicalize(env!("CARGO_BIN_EXE_silo-vmmon"))
                    .expect("silo-vmmon executable")
            );
            assert_eq!(
