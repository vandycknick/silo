...
pub async fn start_services(
                ...
                Ok(stream) => match stream.peer_cred() {
                        ...
                        if crate::vsock::peer::peer_uid_authorized(socket_owner_uid, credentials.uid()) =>
                    {
                        tracing::debug!(
                            ...
                            gid = credentials.gid(),
                            pid = ?credentials.pid(),
                            "accepted silo-vmmon gRPC connection"
                        );
                        Some(Ok(stream))
                    }
                    Ok(credentials) => {
                        tracing::warn!(
                            ...
                            "rejected silo-vmmon gRPC connection from unauthorized UID"
                        );
                        None
                    }
                    Err(error) => {
                        tracing::warn!(%error, "rejected silo-vmmon gRPC connection without peer credentials");
                        None
                    }
                },
                Err(error) => {
                    tracing::warn!(%error, "silo-vmmon gRPC accept failed");
                    Some(Err(error))
                }
            }
        });
    ...
    tracing::info!(
        ...
        "silo-vmmon gRPC control plane is serving"
    );
