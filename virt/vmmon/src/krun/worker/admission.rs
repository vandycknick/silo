...
fn status_guidance(status: i32) -> &'static str {
    match status {
        HV_DENIED => {
            " Hint: rebuild signed silo-vmmon with `cargo run --locked -p xtask -- component silo-vmmon` to grant com.apple.security.hypervisor, and verify sandbox policy permits Hypervisor.framework"
        }
...
mod tests {
    ...
    fn denied_status_names_the_required_entitlement() {
        ...
        assert!(status_guidance(HV_DENIED)
            .contains("cargo run --locked -p xtask -- component silo-vmmon"));
    }
