...
#[command(name = "silo-vmmon", disable_help_subcommand = true)]
struct Args {
...
fn main() -> eyre::Result<()> {
    ...
    tracing_subscriber::fmt()
        ...
        .with_writer(writer)
        .try_init()
        .map_err(|err| eyre::eyre!("initialize silo-vmmon tracing: {err}"))?;

    tracing::info!(
        event = "generation_start",
        machine_id = %args.id,
        run_id = %args.run_id,
        "silo-vmmon generation started"
    );
