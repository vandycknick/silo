...
/// selection means the platform default. Selecting "mock" in a silo-vmmon built
...
fn resolve_backend(
        ...
        Some(backend) if backend.kind == "mock" => {
            ...
            #[cfg(not(feature = "mock-backend"))]
            {
                return Err(crate::virt::VirtError::UnsupportedBackend {
                    kind: "mock",
                    reason: "silo-vmmon was built without the mock-backend feature".to_string(),
                }
                .into());
            }
        }
    ...
    if !crate::virt::BackendKind::compiled().contains(&kind) {
        return Err(crate::virt::VirtError::UnsupportedBackend {
            kind: kind.name(),
            reason: "backend is not compiled into this silo-vmmon binary".to_string(),
        }
        .into());
    }
    Ok(kind)
    ...
    if !requested {
        return match request {
            ...
            Some(_) => Err(eyre::eyre!(
                "silo-vmmon start request enables Rosetta but the durable VM spec disables it"
            )),
        };
    }
...
mod tests {
    ...
    async fn malformed_start_request_fails_before_vm_spec_or_vmm_construction() {
        ...
        assert!(error.to_string().contains("parse silo-vmmon start request"));
