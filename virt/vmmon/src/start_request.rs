...
impl StartRequestPipe {
    ...
    pub(crate) async fn read(
        ...
        tracing::info!(
            ...
            "waiting for silo-vmmon start request"
        );
        ...
        tracing::info!(
            ...
            "silo-vmmon start request accepted"
        );
        Ok(request)
    }
}
...
fn decode_start_request(
    ...
    if encoded.is_empty() {
        return Err(io::Error::new(
            ...
            "silo-vmmon start request is empty",
        ));
    }
    if encoded.len() > VMMON_START_REQUEST_MAX_BYTES {
        return Err(invalid_data(format!(
            "silo-vmmon start request exceeds {} bytes",
    ...
    if encoded.last() != Some(&b'\n') {
        return Err(io::Error::new(
            io::ErrorKind::UnexpectedEof,
            "silo-vmmon start request is not newline terminated",
        ));
    }
        ...
        .any(|byte| matches!(byte, b'\n' | b'\r'))
    {
        return Err(invalid_data(
            "silo-vmmon start request contains a second record or embedded newline",
        ));
    }
    let request: VmmonStartRequest = serde_json::from_slice(&encoded[..encoded.len() - 1])
        .map_err(|error| invalid_data(format!("parse silo-vmmon start request: {error}")))?;
...
fn validate_start_request(
    ...
    if request.version != VMMON_START_REQUEST_VERSION {
        return Err(invalid_data(format!(
            "unsupported silo-vmmon start request version {}",
            request.version
        )));
    }
    ...
    if machine_id != parse_uuid("expected machine ID", expected_machine_id)? {
        return Err(invalid_data(
            "silo-vmmon start request machineId does not match --id",
        ));
    }
    if machine_run_id != parse_uuid("expected machine-run ID", expected_machine_run_id)? {
        return Err(invalid_data(
            "silo-vmmon start request machineRunId does not match --run-id",
        ));
    }
...
fn validate_backend_request(backend: &VirtBackendRequest) -> io::Result<()> {
    match backend.kind.as_str() {
        ...
        "krun" | "vz" => Err(invalid_data(format!(
            "silo-vmmon start request backend {:?} cannot include a mock scenario",
            backend.kind
        ))),
        _ => Err(invalid_data(format!(
            "silo-vmmon start request selected unknown virt backend {:?}",
            backend.kind
        ))),
    }
}
