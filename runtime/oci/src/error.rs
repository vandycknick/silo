...
impl OciError {
    ...
    pub(crate) fn flat_ext4(source: disk_image::ext4::FormatError) -> Self {
        Self::FlatExt4 { source }
    }
}
...
fn repository_not_found_error(reference: &str) -> OciError {
    match oci_client::Reference::from_str(reference) {
        Ok(parsed) => OciError::RegistryImageNotFound {
        ...
        Err(_) => OciError::RegistryImageNotFound {
            reference: reference.to_string(),
...
fn image_not_found_error(reference: &str) -> OciError {
    match oci_client::Reference::from_str(reference) {
        Ok(parsed) => {
            if let Some(tag) = parsed.tag() {
                return OciError::RegistryImageTagNotFound {
                    reference: reference.to_string(),
                    message: format!(
            ...
            if let Some(digest) = parsed.digest() {
                return OciError::RegistryImageDigestNotFound {
                    reference: reference.to_string(),
                    message: format!(
            ...
            OciError::RegistryImageNotFound {
                reference: reference.to_string(),
                message: format!(
                    "repository {:?} does not exist on registry {:?}",
                    parsed.repository(),
...
mod tests {
    use oci_client::errors::{
        OciDistributionError, OciEnvelope, OciError as RegistryEnvelopeError, OciErrorCode,
    };
    ...
    fn manifest_404_reports_missing_digest() {
        let err = OciError::registry_manifest(
    ...
    fn registry_error(code: OciErrorCode, message: &str) -> OciDistributionError {
        OciDistributionError::RegistryError {
            ...
            envelope: OciEnvelope {
                errors: vec![RegistryEnvelopeError {
                    code,
