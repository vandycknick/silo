...
use disk_image::ext4::{FormatOptions, Formatter};
...
impl Ext4Writer {
    pub(crate) fn create(path: &Path, size_bytes: u64) -> OciResult<Self> {
        let formatter = Formatter::with_options(path, FormatOptions::new(size_bytes))
            .map_err(OciError::flat_ext4)?;
        Ok(Self { formatter })
    }

    pub(crate) fn mkdir_p(&mut self, path: &str, mode: u32, uid: u32, gid: u32) -> OciResult<()> {
        self.formatter
            .create(
                path,
    ...
    pub(crate) fn link(&mut self, path: &str, target: &str) -> OciResult<()> {
        self.formatter
            .link(path, target)
            .map_err(OciError::flat_ext4)
    }

    pub(crate) fn delete(&mut self, path: &str) -> OciResult<()> {
        self.formatter.remove(path).map_err(OciError::flat_ext4)
    }

    pub(crate) fn clear_dir(&mut self, path: &str) -> OciResult<()> {
        self.formatter
            .remove_children(path)
            .map_err(OciError::flat_ext4)
    }

    pub(crate) fn finish(self) -> OciResult<()> {
        self.formatter.close().map_err(OciError::flat_ext4)
    }
}
