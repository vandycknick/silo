    ...
    loop {
        tokio::select! {
            ...
            accepted = listener.accept() => match accepted {
                ...
                Err(error) => tracing::warn!(forward = %entry.name, %error, "raw outbound forward accept failed"),
            },
            completed = connections.join_next(), if !connections.is_empty() => {
                if let Some(Err(error)) = completed {
                    tracing::warn!(forward = %entry.name, %error, "raw outbound forward task failed");
                }
            }
        }
    }
