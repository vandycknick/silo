...
pub(crate) struct Vmmon {
    paths: LocalPaths,
    executable: PathBuf,
    krun_executable: PathBuf,
}

impl Vmmon {
    /// Creates a vmmon adapter bound to the runtime's local paths.
    pub(crate) fn new(paths: LocalPaths, executable: PathBuf, krun_executable: PathBuf) -> Self {
        Self {
            paths,
            executable,
            krun_executable,
    ...
    pub(crate) fn krun_executable(&self) -> &std::path::Path {
