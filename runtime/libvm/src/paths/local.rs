...
/// The two roots of a local runtime.
///
/// ```text
/// <home>/            persistent state (~/.silo or SILO_HOME)
///   state.db  machines/<id>/  logs/machines/<id>/  images/  keys/
/// <run_root>/        generated sockets, pidfiles, locks (/tmp/silo-<euid>)
///   machines/<id>/  networks/<nid>/  locks/
/// ```
...
impl LocalRoots {
    ...
    pub(crate) fn new(home: impl Into<PathBuf>) -> Self {
        let home = home.into();
        let run_root = sibling_test_root(&home, "run");
        Self::with_roots(home, run_root)
    }

    pub(crate) fn with_roots(home: impl Into<PathBuf>, run_root: impl Into<PathBuf>) -> Self {
        Self {
            home: home.into(),
    ...
    pub(crate) fn state_db_path(&self) -> PathBuf {
        self.home.join(STATE_DB_FILE_NAME)
    }

    #[cfg(test)]
    pub(crate) fn machines_dir(&self) -> PathBuf {
        self.home.join(MACHINES_DIR_NAME)
    }

    pub(crate) fn images_dir(&self) -> PathBuf {
        self.home.join(IMAGES_DIR_NAME)
    }
...
#[cfg(test)]
fn sibling_test_root(home: &Path, kind: &str) -> PathBuf {
    let parent = home.parent().unwrap_or_else(|| Path::new("/tmp"));
    let name = home
        .file_name()
...
impl LocalPaths {
    ...
    pub(crate) fn machine(&self, machine_id: MachineId) -> MachinePaths {
        MachinePaths::new(self.roots.home(), self.roots.run_root(), machine_id)
    }
    ...
    pub(crate) fn create_machine_data_dir(
        ...
        let root = OwnedDirectory::open_root(self.roots.home())?;
    ...
    fn open_machine_log(
        ...
        let Some(root) = OwnedDirectory::open_existing_root(self.roots.home())? else {
            return Ok(None);
        };
        let Some(logs) = root.open_dir(LOGS_DIR_NAME)? else {
            return Ok(None);
...
mod tests {
    ...
    fn local_roots_derive_expected_subpaths() {
        let roots = LocalRoots::with_roots("/home/me/.silo", "/tmp/silo-501");

        assert_eq!(roots.home(), PathBuf::from("/home/me/.silo").as_path());
        assert_eq!(roots.run_root(), PathBuf::from("/tmp/silo-501").as_path());
        assert_eq!(
            roots.state_db_path(),
            PathBuf::from("/home/me/.silo/state.db")
        );
        assert_eq!(
            roots.machines_dir(),
            PathBuf::from("/home/me/.silo/machines")
        );
        assert_eq!(roots.images_dir(), PathBuf::from("/home/me/.silo/images"));
        assert_eq!(roots.net_dir(), PathBuf::from("/tmp/silo-501/networks"));
        assert_eq!(roots.locks_dir(), PathBuf::from("/tmp/silo-501/locks"));
    }
    ...
    fn local_paths_build_machine_and_network_paths() {
        ...
        assert_eq!(
            ...
            PathBuf::from("/tmp/silo/logs/machines").join(machine_id.to_string())
        );
        assert_eq!(
    ...
    fn default_socket_paths_fit_unix_socket_limits() {
        ...
        let roots = LocalRoots::with_roots(
            "/Users/a-rather-long-account-name/.silo",
            format!("/tmp/silo-{uid}"),
        );
