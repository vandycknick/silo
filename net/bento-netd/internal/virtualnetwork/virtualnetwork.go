...
func New(configuration *types.Configuration, route *router.Router, metadata Metadata) (*VirtualNetwork, error) {
	...
	mux, err := addServices(configuration, stack, ipPool, route, metadata)
	if err != nil {
...
func addServices(configuration *types.Configuration, s *stack.Stack, ipPool *tap.IPPool, route *router.Router, metadata Metadata) (http.Handler, error) {
	var natLock sync.Mutex
	translation := parseNATTable(configuration)

	tcpForwarder := forwarder.TCP(s, translation, &natLock, configuration.Ec2MetadataAccess, route, forwarder.TCPMetadata(metadata))
